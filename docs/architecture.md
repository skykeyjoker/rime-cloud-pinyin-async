...
4. 防抖完成后，helper 把请求分别提交给搜狗和 Google 的单 worker；worker 忙碌
   时只保留最新 pending 请求，不创建无界任务。
5. 第一个来源完成后，helper 再次读取当前请求编号。仍有效才写 revision 1，并通过平台刷新通道通知前端。
6. Lua 收到通知后核对请求编号、输入、候选菜单和选择位置，全部一致才刷新未确认组句。
7. 第二个来源在硬截止前完成后，helper 去重合并并发布 revision 2；若内容没有
   变化则不重复刷新。来源超过 `timeout_ms` 后即使底层 DNS 调用稍后才返回，也只
   会被 worker 丢弃，不再参与当前请求。
...
| `timeout_ms` | helper | provider 参与当前请求的硬截止；不承诺终止操作系统 DNS 调用 |
...
Windows helper 在方案初始化时按需启动，通过 session-local mutex 保证单实例。
主循环每 50 ms 检查请求文件元数据，仅在变化时读取内容，并每 2 秒写一次心跳；
它不执行或等待 HTTP。搜狗和 Google 各有一个低优先级长期 worker，任一 worker
阻塞时只占用自身线程且只保留最新 pending 请求，因此线程数和积压量有固定上限。
helper 目前没有空闲自动退出：小狼毫退出后仍可能驻留，直到注销、重启、手动结束
或发生主循环级致命异常。

Lua 将 6 秒内更新过的心跳视为存活。启动检查只发生在 processor 初始化；请求写入、
候选刷新和重复补查都不运行 `os.execute`。helper 真正退出后，本地输入继续工作，
下一次切换方案、重新部署或重启小狼毫时再尝试拉起。

Windows 弱网回归使用 helper 的隔离测试模式：Google worker 模拟阻塞 10 秒，协调器
硬截止为 500 ms；测试要求两次搜狗结果均在 2 秒内发布、心跳继续前进且不注入
全局 `F24`。测试模式不访问云端点。

## macOS 共享 helper 生命周期

Swift helper 由 Lua 在方案初始化时按需启动，通过 Rime 用户目录中的 `flock` 锁保证
单实例。它每 20 ms 检查请求文件、每 2 秒写一次心跳；Lua 采用 6 秒存活窗口和
5 秒重复启动节流，但输入更新、候选刷新和重复补查都不会执行 `os.execute`。
helper 当前没有空闲退出，前端重启后原 helper 可以继续复用；若它在会话中退出，
本地输入继续工作，并在下一次 engine 初始化时重试。
...
- helper 不存在或崩溃：本地输入继续工作；Windows 与 macOS 都在下一次方案初始化
  时尝试拉起，不在普通输入路径创建进程。
